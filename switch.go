...
var switchCommand = cli.Command{
	Name:  "switch",
	Usage: "switch a running container from a previous checkpoint",
	ArgsUsage: `<container-id>

Where "<container-id>" is the name for the instance of the container to be
restored.`,
	Description: `Restores the saved state of the container instance that was previously saved
using the runc checkpoint command to a RUNNING container.`,
	Flags: []cli.Flag{
		cli.StringFlag{
			Name:  "console-socket",
			Value: "",
		...
		cli.IntFlag{
			Name:  "original-pid",
			Usage: "The running container's init process id",
		},
	},
	Action: func(context *cli.Context) error {
		...
		options := criuSwitchOptions(context)
		if err := setEmptyNsMask(context, options); err != nil {
			return err
		}
		status, err := switchContainer(context, options)
		if err != nil {
			return err
		}
...
func criuSwitchOptions(context *cli.Context) *libcontainer.CriuOpts {
